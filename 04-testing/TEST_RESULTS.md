...
> Generated 2026-08-21 from live pytest execution scoped to project tests.
> Source of truth: `04-testing/coverage_raw.txt` (the raw `pytest --cov` output, scoped to `03-development/tests/`).
...
| Coverage invocation | `pytest 03-development/tests/ --cov=03-development/src --cov-report=term-missing -q --tb=line --no-header` |
| Scope intent | Only the project's own test tree (`test_fr01.py` … `test_fr10.py`, `tests/integration/`, `test_nfr_spec_coverage.py`, `test_bug_hunt_regressions.py`, `test_perf_benchmarks.py`). NO harness vendored tests were collected. |

Running from the repository root without scoping also picks up the framework's vendored copy inside `harness/` and inflates collected test counts into the thousands (measured at `4 failed, 7563 passed, 3 skipped` in a 349-test project tree last cycle). That figure is unrelated to this project's deliverable, so the run is explicitly scoped to `03-development/tests/`.

## Verbatim pytest summary line

Run with coverage instrumentation enabled (the same run that produced `coverage_raw.txt`):

```
23 failed, 240 passed, 1 skipped, 12 warnings in 372.04s (0:06:12)
```

Wall time: 372.04 seconds (6 minutes 12 seconds).
...
| **Total collected cases** | **264** |

**All 23 failures are concentrated in one file** — `03-development/tests/test_nfr_spec_coverage.py` (the NFR acceptance-check suite). The 10 FR-acceptance suites (`test_fr01.py` … `test_fr10.py`), the integration suite, the bug-hunt regression suite, and the perf benchmarks all pass.
...
FAILED 03-development/tests/test_nfr_spec_coverage.py::test_ac_n10_1_integration_line_coverage_at_least_80_percent
...
| NFR-03 | `test_ac_n3_5_timeout_kills_child_awaits_exit_no_orphan_pid` | Test imports `psutil`, which is not in the project's `requirements.txt` (transitive unmet). |
...
| NFR-10 | `test_ac_n10_1_integration_line_coverage_at_least_80_percent` | Integration suite line coverage measured at 79%, threshold is 80% (off-by-one on the integration branch). |
...
| NFR-12 | `test_ac_n12_2_make_verify_system_exits_zero_stdout_contains_pass` | `make verify-system` did not complete within the 300-second subprocess timeout — the verify target is too slow / hangs. |

Total: **23 deferred NFR acceptance checks**. All 10 FR suites, the integration suite, the bug-hunt regression suite, and the perf benchmarks pass.
...
This document records `240 passed + 23 failed + 1 skipped = 264 collected cases`. The harness's own `run_suite` invocation scoped to `03-development/tests/` must produce the same total. The skipped+1 case is the deterministic `pytest.skip` introduced in the latest test set; the 23 failures are all in `test_nfr_spec_coverage.py` and not in any other FR / integration suite.
...
./venv/bin/python -m pytest 03-development/tests/ --cov=03-development/src --cov-report=term-missing -q --tb=line --no-header
```

Expected final line: `23 failed, 240 passed, 1 skipped, 12 warnings in 372.04s (0:06:12)`.
